...
contract("EthSwap", ([deployer, investor]) => {
  ...
  describe("sellTokens", async () => {
    ...
    it("Allows user to instantly sell tokens to ethSwap for a fix price", async () => {
        let investorBalance = await token.balanceOf(investor)
        assert.equal(investorBalance.toString(), tokens('0'))

        let ethSwapBallance = await token.balanceOf(ethSwap.address)
        assert.equal(ethSwapBallance.toString(), tokens('1000000'))
        ethSwapBallance = await web3.eth.getBalance(ethSwap.address)
        assert.equal(ethSwapBallance.toString(), web3.utils.toWei('0', "ether"))
        ...
        await ethSwap.sellTokens(tokens('500'), {from: investor})
          .should
          .be
          .rejected;
    });
